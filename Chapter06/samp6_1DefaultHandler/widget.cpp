...
{
    // 在窗口上点击鼠标，触发事件处理函数mousePressEvent
    if(event->button() == Qt::LeftButton){
        QPoint pt = event->pos();   // 点击在窗口上的相对坐标
        QPointF relaPt = event->localPos(); // 相对坐标
        QPointF winPt = event->windowPos();
        QPointF globPt = event->screenPos();   // 屏幕或虚拟桌面上的绝对坐标
